...
export const GET = withApiHandler(async (req) => {
  ...
  const whereClause = search
    ? `WHERE u.rol = 'prestador' AND (u.nombre ILIKE $3 OR u.apellidos ILIKE $3 OR u.email ILIKE $3)`
  ...
  const [rows, countResult] = await Promise.all([
    query(
      `SELECT
         ...
         u.activo, u.recibe_ordenes, u.created_at, u.avatar_url,
         COALESCE(ROUND(AVG(c.puntuacion)::numeric, 1), 0) AS calificacion_promedio,
