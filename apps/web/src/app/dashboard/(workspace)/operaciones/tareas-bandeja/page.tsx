...
import { getPrestadoresCercanos } from '@/lib/dashboard/queries/operaciones-workspace';
import { formatFechaHora } from '@/lib/dashboard/format';

export const dynamic = 'force-dynamic';
export const metadata: Metadata = {
  title: 'Tareas de soporte · Operaciones · ExpressMX',
};
...
export default async function OperacionesTareasBandejaPage() {
  const viewer = await requirePermiso('ordenes.editar');
  const puedeCompletar = tienePermiso(viewer, 'soporte.tarea.completar');

  const tareas = await listarTareasPorGrupo('operaciones_l1', { limit: 100, soloActivas: true });

  const prestadoresPorOrden = new Map<string, Awaited<ReturnType<typeof getPrestadoresCercanos>>>();
  for (const t of tareas) {
    if (t.tipo === 'reasignar_prestador') {
      const ordenId = typeof t.payload['orden_id'] === 'string' ? (t.payload['orden_id'] as string) : null;
      if (ordenId && !prestadoresPorOrden.has(ordenId)) {
        try {
          const candidatos = await getPrestadoresCercanos(ordenId, 8);
          prestadoresPorOrden.set(ordenId, candidatos);
        } catch {
          prestadoresPorOrden.set(ordenId, []);
        }
      }
    }
  }
    ...
    <WorkspaceShell
      queue={
        <nav className="flex flex-col gap-1 p-3 text-sm">
          ...
          <Link
            href="/dashboard/operaciones/tareas-bandeja"
        ...
        tareas.length === 0 ? (
          <div className="rounded-md border border-dashed border-border bg-background p-8 text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Cuando soporte solicite reasignación de prestador u otra acción operativa,
              aparecerá aquí.
            </p>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1 space-y-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="rounded-md border border-warning/40 bg-warning/10 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-warning-foreground">
                        {TIPO_LABEL[t.tipo]}
                      </span>
                  ...
                  {puedeCompletar ? (
                    <TareaCompletarForm
                      ...
                      prestadoresCandidatos={
                        t.tipo === 'reasignar_prestador' &&
                        typeof t.payload['orden_id'] === 'string'
                          ? prestadoresPorOrden.get(t.payload['orden_id'] as string) ?? []
                          : undefined
                      }
                    />
                  ) : null}
          ...
          sections={[
            {
              ...
              children: (
                <ul className="space-y-1 text-[11px] text-muted-foreground">
                  <li>
                    <strong>Reasignar prestador:</strong> identificar prestador alternativo en zona, abrir nueva orden y notificar al cliente.
                  </li>
                  <li>
                    <strong>Investigación operacional:</strong> revisar logs de orden, asistencia, geofencing.
                  </li>
                </ul>
              ),
            },
