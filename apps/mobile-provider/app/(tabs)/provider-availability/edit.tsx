...
export default function ProviderAvailabilityEditScreen() {
  const router = useRouter();

  return (
    <ScreenShell>
      <ScreenHeader title="Horario asignado" subtitle="Gestionado por Operaciones" />
      <Box className="flex-1 items-center justify-center px-6">
        <VStack className="items-center gap-3">
          <Box className="w-14 h-14 rounded-xl bg-primary-soft items-center justify-center">
            <CalendarBlank size={28} color={palette.brandStrong} weight="duotone" />
          </Box>
          <Text className="text-base font-semibold text-foreground text-center">
            Tus turnos se administran desde ExpressMX.
          </Text>
          <Text className="text-sm text-foreground-secondary text-center">
            Cualquier ajuste queda a cargo de RRHH u Operaciones.
          </Text>
        </VStack>
      </Box>
      <BottomBar>
