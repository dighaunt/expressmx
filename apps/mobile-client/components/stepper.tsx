    ...
    <VStack className={className ?? ''} onLayout={handleLayout}>
      <HStack className="items-start">
        {steps.map((step, index) => {
          ...
          const circleStyle = reached
            ? { backgroundColor: palette.brand, borderColor: palette.brand }
            : { backgroundColor: palette.surface, borderColor: palette.borderStrong };
          const numberColor = reached ? palette.surface : palette.textTertiary;
          const labelColor = reached ? palette.brand : palette.textTertiary;
          const labelWeight = reached ? '600' : '400';
              ...
              <HStack className="items-center w-full">
                {isFirst ? (
                  ...
                  <Box
                    ...
                    style={{
                      backgroundColor: leftLineCompleted ? palette.brand : palette.surfaceSubtle,
                    }}
                  />
                )}
                <Box
                ...
                >
                  {isCompleted ? (
                    <Check size={14} color={palette.surface} weight="fill" />
                  ) : (
                    <Text className="text-xs font-bold" style={{ color: numberColor }}>
                      {index + 1}
                    </Text>
                  )}
                </Box>
                {isLast ? (
                  ...
                  <Box
                    className="flex-1 h-0.5"
                    style={{
                      backgroundColor: rightLineCompleted ? palette.brand : palette.surfaceSubtle,
                    }}
                  />
                )}
              </HStack>
              <Text
                className="text-xs mt-1.5 px-1 text-center"
