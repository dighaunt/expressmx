...
import { subscribeTicketRealtime, type RealtimeEvent } from '@/lib/realtime';
...
function messageFromRealtimeEvent(event: RealtimeEvent): Mensaje | null {
  if (
    event.name !== 'ticket.message_created' ||
    event.data.internal === true ||
    !event.data.messageId ||
    !event.data.content ||
    !event.data.authorType ||
  ...
  return {
    ...
    contenido: event.data.content,
    tipo_autor: event.data.authorType,
    created_at: event.data.createdAt,
  };
}

export default function ClientTicketDetailScreen() {
  ...
  const ticketEstatus = ticket?.estatus;

  useEffect(() => {
    if (!id || !ticketEstatus || ticketEstatus === 'resuelto') return;
    let cleanup: (() => void) | undefined;
    let cancelled = false;
    subscribeTicketRealtime(id, (event) => {
      if (event.data.ticketId !== id) return;
      if (event.name === 'ticket.updated') {
        void load();
        return;
      }
  ...
  if (loading) {
    return (
      <ScreenShell>
        ...
        <Box style={{ paddingHorizontal: 20, paddingTop: 8 }}>
